...
def make_sno_file(loc_lat,loc_lon,in_i,in_j,loc_hgt,loc_snowh,soil_thickness,soil_T,soil_loc_w,
    ...
    fn_out = "./snpack_" + str(domain) + "_" + str(in_j+1) + "_" + str(in_i+1) + ".sno"
    if(loc_snowh > 0):
        loc_snlayers=10
    else:
        loc_snlayers=0
    ...
    if(loc_snowh > 0):
        for m in range(10):
            file.write(datestr.isoformat()[:] + "   " +
                       "%6.3f" % (loc_snowh/10.0) + "   " +  # layer-thickness [m]
...
wrf_input = xr.open_mfdataset('../wrfinput_d0'+str(domain))
...
datestr = dt.datetime.strptime(wrf_input.attrs['START_DATE'], "%Y-%m-%d_%H:%M:%S")
print(datestr)
...
met_file = xr.open_mfdataset(metdir+'met_em.d0'+str(domain)+'.'+start_date+'.nc')


T_SOIL=np.concatenate((met_file['ST100289'].values, \
                       met_file['ST028100'].values, \
                       met_file['ST007028'].values, \
                       met_file['ST000007'].values),axis=0)

W_SOIL=np.concatenate((met_file['SM100289'].values, \
                       met_file['SM028100'].values, \
                       met_file['SM007028'].values, \
                       met_file['SM000007'].values),axis=0)       

lu_index = met_file['LU_INDEX'].values[0,:,:]    
soil_layer_thickness = np.array([1.89,0.72,0.21,0.07])    

soil_layer_ne = np.array([3,3,3,2])
...
T_SOIL_TOP = met_file['ST000007'].values[0,:,:]
...
for i in range(max_x_wrf):
    for j in range(max_y_wrf):
        ...
        if(lu_index[i,j] == 17 or lu_index[i,j] == 21):
...
#        print( line.replace( line.split("=")[1], ' ' + 'Swiss:WRF' ),end='\n' )
#    elif line.startswith('2016-01-10'):  
